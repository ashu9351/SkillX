...
const Jobs = () => {
  ...
  const filteredJobs = useMemo(() => {
    ...
    return jobs.filter((job) => {
      ...
      if (job.domain === jobCategory) {
        return jobCountry?.toLowerCase() === country.toLowerCase();
      }
      return false;
    });
  }, [jobs, country, jobCategory]);

  const handleApplyNow = (query) => {
    const params = new URLSearchParams();
    params.set("query", query);
    params.set("source", "jobs");
    params.set("jobCategory", jobCategory);
  ...
  return (
    <div className="container" style={{ padding: "2rem 0" }}>
      <div className="page-header">
        <h1 className="page-title">Jobs in {country}</h1>
      ...
      {filteredJobs.length > 0 ? (
        ...
        >
          No jobs found in {country} for {jobCategory}
        </p>
      )}
    </div>
  );
